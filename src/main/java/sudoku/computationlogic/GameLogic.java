...
public class GameLogic {
    ...
    public static boolean sudokuIsInvalid(int[][] grid){
        ...
        else if (columnsAreInvalid(grid)) return true;
        else if (squaresAreInvalid(grid)) return true;
        else return false;
    }
    ...
    private static boolean rowOfSquaresIsInvalid(Rows value, int[][] grid){
        switch(value) {
            case TOP:
                ...
                else if (squareIsInvalid(0,3, grid)) return true;
                else if (squareIsInvalid(0,6, grid)) return true;
                else return false;
            case MIDDLE:
                if (squareIsInvalid(3,0, grid)) return true;
                else if (squareIsInvalid(3,3, grid)) return true;
                else if (squareIsInvalid(3,6, grid)) return true;
                else return false;
            case BOTTOM:
                if (squareIsInvalid(6,0, grid)) return true;
                else if (squareIsInvalid(6,3, grid)) return true;
                else if (squareIsInvalid(6,6, grid)) return true;
                else return false;
